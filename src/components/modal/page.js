...
const RadioInput = ({ id, name, value, label }) => (
  <div className="mb-2">
    <input
      ...
      className="radio visually-hidden"
    />
    <label htmlFor={id} className="radio-label ml-2">{label}</label>
  </div>
);
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-md shadow-md w-4/12">
            <div className="flex items-center  mb-6 flex justify-center">
              <div className="text-green-700 text-sm font-semibold font-inter mr-28 ">
                กรอกรายละเอียดเพื่อคำนวณแคลอรี่ของคุณ
              </div>
              <div
                className="text-blue-500 text-xs font-normal font-inter cursor-pointer"
                onClick={closeModal}
              >
                ข้าม
              </div>
            ...
            <div className="inline-flex items-start justify-center mb-4">
              ...
              <div className="inline-flex flex justify-between ml-28  text-lg">
                <RadioInput id="male" name="gender" value="male" label="ชาย" />
                <RadioInput id="female" name="gender" value="female" label="หญิง" />
              </div>
            </div>
